        ...
        <div class="bg">
            <div class="q-pa-md">
                <div class="row">
                    ...
                    <p class="font">Dashboard Headline</p>
                </div>
                <p class="fontKecil jarak">Daftar Headline</p>
                <div class="full-width bg-grey-8 q-pa-sm text-center text-white" style="border-radius: 10px 10px 10px 10px;">
                    <p class="font q-mt-sm">Dashboard / Headline</p>
                </div>
            </div>
        </div>
        <div>
            <q-card class="q-ma-xl bordered" style="margin-top: -80px">
                <div class="row bg-grey-11" style="max-height: auto;">
                    <div class="col-md-6 col-xs-6 text-blue-14 q-pl-lg q-pt-sm"><p style="font-weight: 400; font-size: 20px;">Daftar Headline</p></div>
                    <div class="col-md-6 col-xs-6">
                        <div v-if="$q.screen.sm || $q.screen.xs">
                            <div class="text-right q-pa-xs">
                                <q-btn icon="add" color="primary" :to="{ name: 'inputHeadline' }" no-caps></q-btn>
                            </div>
                        </div>
                        <div v-if="$q.screen.lg || $q.screen.xl || $q.screen.md" class="text-right">
                            <div class="text-right q-pa-xs">
                                <q-btn label="Tambah Data" color="primary" :to="{ name: 'inputHeadline' }" no-caps></q-btn>
                            </div>
                        </div>
                    </div>          
                </div>
                ...
                >
                    ...
                    <q-tr>
                        ...
                        <q-td key="nama_headline" class="text-center tabelSpaceJudul">
                        {{ props.row.nama_headline }}
                        </q-td>
                        ...
                        <div class="row q-gutter-md">
                            <!-- <p>{{props.row._id}}</p> -->
                            <q-btn :to="{ name: 'formEditHeadline', params: { id: props.row._id }}" label="Edit" icon="edit" color="warning" unelevated/>
...
const columns = [
  ...
  { name: 'nama_headline', align: 'Left', label: 'Nama Headline', field: 'nama' },
  ...
  methods: {
    ...
    deleteMovie (id) {
      this.$q.dialog({
        ...
        this.$api.delete(`headline/delete/${id}`)
          .then(res => {
            if (res.data.sukses) {
