    ...
    <div class="bg-grey-11">
        <div class="row">
            <div class="col-md-6 col-xs-6">
                <q-btn icon="folder" disable color="black" label="Tambah Amenitas" no-caps flat></q-btn>
            </div>
        ...
        <div class="row" style="height: 90vh">
            ...
            <q-card flat bordered>
                <div class="q-pb-sm q-pt-md q-pl-md bg-grey-3">
                    <p style="font-size: 20px; font-weight: 500; color: blue;">Informasi Amenitas</p>
                </div>
                <q-card-section>
                <q-form
                    @submit="onSubmit"
                    ...
                    <q-input outlined label="Nama Amenitas" v-model="nama_amenitas"
                    :rules="[ val => val && val.length > 0 || 'Lengkapi nama Amenitas']"
                    >
                    </q-input>
                    <q-input 
                    outlined 
                    label="Deskripsi Amenitas"
                    type="textarea"
                    v-model="deskripsi"
                    :rules="[ val => val && val.length > 0 || 'Lengkapi deskripsi']"
                    >
                    </q-input>
                    <q-select outlined v-model="jenis" :options="options" label="Jenis Amenitas" />
                    <q-file
                        accept=".jpg, image/*"
                        outlined
                        v-model="image"
                        label="Gambar Amenitas"
                    >
  ...
  methods: {
    onSubmit () {
      ...
      formData.append('data', JSON.stringify({
        nama_amenitas: this.nama_amenitas,
        deskripsi: this.deskripsi,
        jenis: this.jenis
      }))
      this.$api.post('amenitas/tambah', formData).then(res => {
        console.log(res.data)
        if (res.data.code === 404) {
            this.$showNotif('File Harus Berbenruk Gambar', 'negative')
        }
        else if (res.data.code === 200) {
            this.$router.push({ name: 'amenitas' })
